            ...
            <tr>
                ...
                <?php if (in_array($step, $st_nhom_opt)) { ?>
                    <th class="w90 text-center">
                        <?= @$name_list_opti[$step]["op0"] ?>
                    </th>
                <?php } ?>
                <?php if (in_array($step, $st_nhom_opt1)) { ?>
                    <th class="w90 text-center">
                        <?= @$name_list_opti[$step]["op1"] ?>
                    </th>
                <?php } ?>
                <?php if (in_array($step, $st_nhom_opt2)) { ?>
                    <th class="w90 text-center">
                        <?= @$name_list_opti[$step]["op2"] ?>
                    </th>
                <?php } ?>
            ...
            foreach ($sql as $rows) {
                ...
                <tr class="p_hover">
                    ...
                    <td>
                        ...
                        <p class="a_nguoidang">Đăng bởi:
                            <?= $members_dang[$id_user]['hoten'] ?>
                            (
                            <?= $members_dang[$id_user]['tentruycap'] ?>)
                            -
                            <?= date('d-m-Y H:i:s', $rows['ngaydang']) ?>
                        </p>
                        <p class="p_chude_hienthi">
                            <?= ($id_parent == 0) ? '' : $list_danhmuc[$rows['id_parent']]['tenbaiviet_vi'] ?>
                        </p>
                        <p class="p_chude_hienthi" <?php if ($opt3 != 1) { ?> style="display: none;" <?php } ?>>
                            <spam class="r">
                                <?= NUMBER_fomat($giatien) ?>đ
                            </spam>
                            <?= $giakm != 0 ? ' - <span class="km">' . NUMBER_fomat($giakm) . "đ</span>" : "" ?>
                        </p>
                        ...
                        if (in_array($step, $sl_quanlybaiviet)) {
                            ...
                            <p style="margin: 0;">
                                <a href="?module=bai-viet-chi-tiet&action=bai-viet-chi-tiet&id-parent=<?= $id ?>&step=<?= $step ?>"
                                    style="font-size: 12px; color: #ff4c14;">[Quản lý bài viết] [<?= DB_num($so_baiviet) ?>
                                    ]</a>
              ...
              <a href="<?= $url_page . '&del_bl=' . $bcdd['id'] . '&token=' . GET_token() ?>&step=<?= $step ?>&id_step=<?= $id_step ?>" onclick="return confirm('Bạn thật sự muốn xóa?')">[Xóa]</a>
                        ...
                        <div class="dv-nhom-sppp">
                            <?php if (in_array($step, $st_nhom_opt3)) { ?>
                                <label><input opt3 type='checkbox' class='minimal minimal_click' colum="opt3" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt3, 1) ?>> <?= @$name_list_opti[$step]["op3"] ?>
                                </label>
                            <?php } ?>
                            <?php if (in_array($step, $st_nhom_opt4)) { ?>
                                <label><input opt4 type='checkbox' class='minimal minimal_click' colum="opt4" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt4, 1) ?>> <?= @$name_list_opti[$step]["op4"] ?>
                                </label>
                            <?php } ?>
                            <?php if (in_array($step, $st_nhom_opt5)) { ?>
                                <label><input opt5 type='checkbox' class='minimal minimal_click' colum="opt5" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt5, 1) ?>> <?= @$name_list_opti[$step]["op5"] ?>
                                </label>
                            <?php } ?>
                        </div>
                    ...
                    <?php if (in_array($step, $st_nhom_opt)) { ?>
                        <td class="text-center">
                            <div id="cus" class="cus_menu">
                                <label><input opt type='checkbox' class='minimal minimal_click' colum="opt" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt, 1) ?>></label>
                            </div>
                        </td>
                    <?php } ?>
                    <?php if (in_array($step, $st_nhom_opt1)) { ?>
                        <td class="text-center">
                            <div id="cus" class="cus_menu">
                                <label><input opt1 type='checkbox' class='minimal minimal_click' colum="opt1" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt1, 1) ?>></label>
                            </div>
                        </td>
                    <?php } ?>
                    <?php if (in_array($step, $st_nhom_opt2)) { ?>
                        <td class="text-center">
                            <div id="cus" class="cus_menu">
                                <label><input opt2 type='checkbox' class='minimal minimal_click' colum="opt2" idcol="<?= $id ?>"
                                        table="#_baiviet" value='1' <?= LAY_checked($opt2, 1) ?>></label>
                            </div>
                        </td>
                    <?php } ?>
                    ...
                    <td class="text-center">
                        <div id="cus" class="cus_menu">
                            <label><input showhi type='checkbox' class='minimal minimal_click' colum="showhi"
                                    idcol="<?= $id ?>" table="#_baiviet" value='1' <?= LAY_checked($showhi, 1) ?>></label>
                        </div>
                    </td>
                    <td class="text-center">
